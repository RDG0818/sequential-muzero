...
class MCTSSequentialPlanner(MCTSPlanner):
    ...
    def _recurrent_fn(self, params, rng_key, action: chex.Array, embedding: Tuple[chex.Array, chex.Array, chex.Array]) -> Tuple[mctx.RecurrentFnOutput, Tuple]:
            ...
            """
            latent, agent_idx, coordination_info = embedding # latent: (B,N,D), agent_idx: (B,) TODO: make coordination info actually useable
            ...
            new_embed = (next_latent, agent_idx, coordination_info)
    ...
    def _plan_loop(self, params, rng_key, observation: chex.Array):
        ...
        def agent_step(carry: Tuple, inputs: Tuple) -> Tuple[Tuple, Tuple]:
            ...
            emb = (root_latent, jnp.array([agent], jnp.int32), jnp.zeros((1, self.num_agents))) # (1,N,D), (1,) #TODO: This is what is passed to recurrent_fn, update logic for conditioning
